...
class Home extends BaseController
{
	...
	public function deleteUser()
	{
		...
		if(!isset($firstname) || trim($firstname) === '')
		{
			$this->returnError(400,'id à supprimer manquant');
			return;
		}
		$client = $this->getConnection();
	...
	public function listSongs()
	{
		$id = $_GET['id'];
		echo "id".$id;
		echo "getvar".$this->request->getVar('id');
		return;
		if(!isset($id) || trim($id) === '')
		{
			//listSongs
		...
		{
			//song byId
			$getSongByIdQuery = array('_id' => $id);
			$songByID = $db->user->findOne($getSongByIdQuery);
			$arr = array('status' => 200,'error' => "mba testa eeeeee",'song'=> (object)$songByID);
			header('Content-Type:application/json');
			echo json_encode($arr);	
			return;
		}
