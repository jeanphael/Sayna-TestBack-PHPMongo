...
class Home extends BaseController
{
	...
	public function register()
	{
		...
		$insertOneResult = $collection->insertOne(['firstname'=>$firstname,'lastname'=>$lastname,'email'=>$email,'password'=>$pwdCrypt,'date_naissance'=>$date_naissance,'sexe'=>$sexe]);
		if($insertOneResult->getInsertedId() != '0')
		{
			$arr = array('status' => 200,'error' => false,'message' =>'L\'utilisateur a bien été créé avec succès','user'=> (object)$row);
					header('Content-Type:application/json');
					echo json_encode($arr);	
					return;
		}
