...
class Home extends BaseController
{
		...
		if($userByLogin != null)
        {
			foreach ($userByLogin as $row)  {
				...
				if(password_verify($password, $mdps)) {
					...
					//updateToken 
					$userUpdated = $db->user->updateOne($loginQuery,[ '$set' => [ 'token' => $token,'updateAT'=>time()]]);
					$this->response->setStatusCode(200);
					$arr = array('error' => false,'message' =>'L\'utilisateur a été authentifié succès','user'=> (object)$row);
	...
	public function register()
	{
		...
		$insertOneResult = $collection->insertOne(['token'=>'','firstname'=>$firstname,'lastname'=>$lastname,'email'=>$email,'password'=>$pwdCrypt,'token'=>$pwdCrypt,'date_naissance'=>$date_naissance,'sexe'=>$sexe,'createdAt'=>time(),'updateAT'=>time(),'subscription'=>'']);
