...
class Home extends BaseController
{
	...
	public function addCart()
	{
		...
		foreach($arrayParameter as $c)
		{
			...
			if(str_contains($c,'month'))$month = substr($c, 0, strpos($c,':'));
			if(str_contains($c,'year'))$year = substr($c, 0, strpos($c,':'));
			if(str_contains($c,'default'))$default = substr($c, 0, strpos($c,':'));
		}
		echo $cart.$month.$year.$default;
